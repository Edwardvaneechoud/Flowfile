...
Writes docs/users/formulas/functions.md and the function_summary.snippet fragment
that the hand-written overview (index.md) includes for its counts and category
table. The library is the source of truth; rerun via `make formula_docs` after
bumping the polars-expr-transformer pin.
...
# Plural noun each category goes by in the overview page's prose sentence, and a
# few representative functions for its summary-table row. A category missing from
# either map fails the build rather than silently dropping out of index.md.
CATEGORY_PROSE = {
    "Logic & Nulls": "logic",
    "String": "strings",
    "Math": "math",
...
CATEGORY_EXAMPLES = {
    "Logic & Nulls": ["coalesce", "ifnull", "between", "is_empty"],
    "String": ["concat", "uppercase", "trim", "replace", "split"],
    "Math": ["round", "abs", "floor", "power", "log"],
    "Date & Time": ["year", "add_days", "date_diff_days", "format_date"],
    "Type Conversion": ["to_string", "to_integer", "to_date", "to_boolean"],
    "Encoding": ["base64_encode", "hex_encode", "encode", "decode"],
    "Hashing": ["sha256", "md5", "hash", "sha512"],
}
...
SUMMARY_PATH = OUTPUT_PATH.parent / "function_summary.snippet"
...
def build_summary() -> str:
    """Render the counts/category fragment index.md includes via pymdownx.snippets.

    Keeping these numbers generated is what stops the hand-written overview from
    drifting when the upstream library gains or loses functions.
    """
    populated = [(title, collect_functions(module)) for title, module in CATEGORIES]
    populated = [(title, functions) for title, functions in populated if functions]
    total = sum(len(functions) for _, functions in populated)

    rows = []
    for title, functions in populated:
        names = {name for name, _ in functions}
        missing_meta = [m for m in (CATEGORY_PROSE, CATEGORY_EXAMPLES) if title not in m]
        if missing_meta:
            raise SystemExit(f"Add {title!r} to CATEGORY_PROSE and CATEGORY_EXAMPLES in {Path(__file__).name}")
        unknown = [e for e in CATEGORY_EXAMPLES[title] if e not in names]
        if unknown:
            raise SystemExit(f"CATEGORY_EXAMPLES[{title!r}] names functions that no longer exist: {unknown}")
        examples = ", ".join(f"`{e}`" for e in CATEGORY_EXAMPLES[title])
        rows.append(f"| [{title}](functions.md#{slugify(title)}) | {len(functions)} | {examples} |")

    categories = _prose_list([CATEGORY_PROSE[title] for title, _ in populated])
    ...
    return "\n".join(
        [
            ...
            "",
            "--8<-- [start:total]",
            f"There are {total} built-in functions for {categories} — see the [function reference](functions.md).",
            "--8<-- [end:total]",
            "",
            "--8<-- [start:table]",
