...
class PrepareUploadRequest(BaseModel):
    ...
    serialization_format: str = Field(..., description="Serialization format: parquet, joblib, or pickle")
    description: str | None = Field(None, description="Human-readable description of the artifact")
    tags: list[str] = Field(default_factory=list, description="Tags for categorization and search")
    namespace_id: int | None = Field(
        None, description="Namespace (schema) ID. Defaults from source registration if not provided."
    )

    # Lineage information (set by kernel)
    source_flow_id: int | None = Field(None, description="ID of the flow that created this artifact")
    source_node_id: int | None = Field(None, description="ID of the node that created this artifact")
    source_kernel_id: str | None = Field(None, description="ID of the kernel that created this artifact")

    # Type information (set by kernel after serialization)
    python_type: str | None = Field(
        None, description="Full Python type name (e.g., 'sklearn.ensemble.RandomForestClassifier')"
    )
    python_module: str | None = Field(None, description="Python module name (e.g., 'sklearn.ensemble')")
...
class PrepareUploadResponse(BaseModel):
    ...
    method: str = Field(..., description="Upload method: 'file' for local filesystem or 's3_presigned' for S3")
    path: str = Field(..., description="Local path or presigned URL where kernel should write")
    storage_key: str = Field(..., description="Storage key to include in finalize request")
...
class DownloadSource(BaseModel):
    ...
    method: str = Field(..., description="Download method: 'file' for local filesystem or 's3_presigned' for S3")
    path: str = Field(..., description="Local path or presigned URL where kernel should read from")
