...
def trigger_resolve_virtual_table(
    ...
    """Ask the worker to materialise a flow-virtual table to disk.

    Ships *plan_bytes* (output of ``pl.LazyFrame.serialize()``); the worker
    deserialises in a spawned child, collects, and writes IPC. Each call
    re-executes the plan; the worker holds no cache.
    """
