...
def _apply_add_missing(
    ...
    expressions = [
        _parse_default_value(field).alias(field.name) for field in fields if field.name not in current_columns
    ]
    if expressions:
...
def _validate_data_types(df: FlowDataEngine, fields: list[OutputFieldInfo]) -> None:
    ...
    for field in fields:
        ...
        if column.data_type != field.data_type:
            mismatches.append(f"Column '{field.name}': expected {field.data_type}, got {column.data_type}")
...
def apply_output_field_config(
    ...
    try:
        ...
        if mode == "raise_on_missing":
            ...
            new_flow_engine = flow_data_engine.select_columns([field.name for field in output_field_config.fields])
        else:
