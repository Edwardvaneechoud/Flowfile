...
NODE_LONG_DESCRIPTIONS: Final[dict[str, str]] = {
    ...
    "gate": (
        ...
        "itself. ${param} refs work inside the formula. Use for "
        "environment-gated writes, data-quality gates (gate on the data itself, "
        "or a 'filter' feeding the control input), and either/or branches. "
        "Don't use to drop rows — that's 'filter'; don't use for pure ordering "
        "with no condition — that's 'wait_for'. Example: "
        '{"gate_input": {"condition_source": "parameter", "parameter": "env", '
        '"operator": "equals", "value": "prod"}}. '
        "Canonical pattern: put branch A behind a gate and branch B behind a "
        "second gate with the complementary condition (not_equals / is_false / "
        "not_in, or a negated formula), then re-converge both into a 'union' — "
        "the gated-off branch contributes a zero-row frame with its schema, so "
        "the union emits whichever branch survived. Cost note: a gate only "
        "prevents its DOWNSTREAM from running, so place it as early as possible."
    ),
    "wait_for": (
...
NODE_USER_INSTRUCTIONS: Final[dict[str, str]] = {
    ...
    "gate": (
        ...
        "default) or a formula. For a parameter condition pick the "
        "'Parameter', an 'Operator' (equals / not equals / in / not in / is "
        "true / is false / is set) and a 'Value'. For a formula condition "
        "write a flowfile formula (the Filter node's expression language, "
        "e.g. [status] = 'error') — the gate opens when at least one row "
        "matches it, checked against the optional control input when one is "
        "connected, otherwise against the data input itself. Worked example: "
        "'only load to the production table when I run with env=prod' → drag "
        "'Gate' from Combine Operations, connect the cleaned data to the "
        "first input, set Parameter=env, Operator=equals, Value=prod, then "
        "connect the gate to 'Write to Database'. To run one branch or the "
        "other, add a second 'Gate' with the complementary condition (not "
        "equals, or a negated formula), put the alternative branch behind it, "
        "and feed both branches into a 'Union data' node — the closed branch "
        "contributes no rows, so the union yields whichever branch ran. "
