...
1. Explicit ``model=`` argument (caller knows best).
2. The credential row's ``default_model`` (user-configured override).
3. The provider class' D010 ``surface_models[surface]`` map.
4. The provider class' ``default_model``.
...
from flowfile_core.ai.credentials import decrypt_api_key, get_provider_credential
