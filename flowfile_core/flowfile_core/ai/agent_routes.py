...
* ``POST /ai/agent/start`` — open a session, stream :class:`PlannerEvent`s
  ...
  abort/resume routes can find it; the SSE stream itself is the
  planner's primary output.
* ``POST /ai/agent/{session_id}/resume`` — body ``{action: "continue" |
  "discard"}``. ``"continue"`` re-snapshots the graph and resumes the
  generator (SSE response). ``"discard"`` pops the session and frees
  any staged diff (JSON response).
* ``POST /ai/agent/{session_id}/abort`` — flip status to ``aborted``
  so the in-flight generator's next iteration exits cleanly. JSON
  response. Idempotent: aborting an already-completed/aborted session
  is a no-op.
* ``GET /ai/agent/{session_id}`` — status snapshot for re-attach (the
  frontend uses this on page-reload to re-render the in-progress
  state).
...
class AgentFollowupRequest(BaseModel):
    """Body for ``POST /ai/agent/{session_id}/followup``.
...
async def agent_start(
    ...
    * ``422`` — ``surface="agent_complex"`` on a ``supports_tools=False``
    ...
    # One structured INFO line per session start. Pairs with the
    # ``planner.staged session=…`` lines emitted on each stage
    # transition, so a single ``grep agent\\.`` against the core's
    # stderr/log file reconstructs every session's surface + provider +
    # model + the full state-machine trajectory without parsing the
    # JSONL prompt log.
    logger.info(
...
async def agent_resume(
    ...
    * ``action="continue"`` re-snapshots the graph and streams the rest
      of the planner's events as SSE (mirrors ``/start``). When the
      request carries a ``Last-Event-ID`` header, buffered SSE frames
      newer than the cursor are flushed to the client *before* the live
      planner stream resumes.
    * ``action="discard"`` pops the session, returns JSON. Any staged
      diff that was registered before the pause stays — the user can
      still reject it via the ``/ai/diff/{id}/reject`` route.

    Resumable statuses: ``paused_drift`` and ``paused_user_action``
    (cold-start). Other statuses → 409.
...
async def agent_followup(
    ...
    """Re-enter a completed planner session with a synthetic followup.
    ...
    4. Calls :func:`run_planner_session` — the planner's followup-resume
       ...
       graph, and re-enters the loop with the appended message.
...
async def agent_state(
    ...
    Cold-start hydration: when the read pulls a ``running`` session
