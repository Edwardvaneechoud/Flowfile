...
_LLM_SYSTEM_PROMPT = (
    ...
    "or steps in their flow. This includes follow-ups that confirm a prior "
    "assistant suggestion (e.g. 'yes do it', 'implement that', 'apply', "
    "'can you implement?'). Imperatives like 'add a sort node' count too.\n"
    ...
    "context to choose.\n\n"
    "Conversation context is decisive. A short message like 'yes please', "
    "'go ahead', or 'can you implement?' that follows an assistant turn "
    "suggesting concrete nodes / steps is build intent — the user is asking "
    "the assistant to execute the suggestion. The same words with no prior "
    "build-shaped suggestion are ambiguous.\n\n"
