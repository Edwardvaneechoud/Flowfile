...
def _cloud_row(name: str):
    from flowfile_core.database.models import CloudStorageConnection
...
def test_cloud_connection_profile_round_trip(tmp_path):
    ...
    try:
        with get_db_context() as db:
            store_cloud_connection(
                ...
                FullCloudStorageConnection(
                    connection_name=cli, storage_type="s3", auth_method="aws-cli", aws_profile="analytics"
                ),
                OWNER,
            )
        ...
        legacy = {k: v for k, v in cli_file.items() if k != "aws_profile"}
        _import_cloud_connection(legacy, OWNER, {}, SetupResult())
