...
other_df = df.select(
    ...
    pl.col('salary').over('city'), 'other_df')
...
df1 = pl.FlowFrame({"a": [1], "b": [3]})
df2 = pl.FlowFrame({"a": [2], "b": [4]}, flow_graph=df1.flow_graph)
df1.concat([df2], how='diagonal_relaxed')
