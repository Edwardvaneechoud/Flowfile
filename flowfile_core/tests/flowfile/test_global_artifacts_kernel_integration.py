...
class TestGlobalArtifactsKernelRuntime:
    ...
    def test_publish_global_basic(self, kernel_manager_with_core: tuple[KernelManager, str], cleanup_global_artifacts, test_registration):
        ...
        code = '''
artifact_id = flowfile_ctx.publish_global("kernel_test_model", {"accuracy": 0.95, "type": "classifier"})
...
# Verify it exists
obj = flowfile_ctx.get_global("to_delete")
assert obj["temp"] is True

# Delete
flowfile_ctx.delete_global_artifact("to_delete")

# Verify it's gone
try:
    flowfile_ctx.get_global("to_delete")
...
'''
    ...
    def test_get_nonexistent_raises_key_error(
        ...
        code = '''
try:
    flowfile_ctx.get_global("definitely_does_not_exist_12345")
...
'''
    ...
    def test_versioning_on_republish(
        ...
        code = '''
# Publish v1
id1 = flowfile_ctx.publish_global("versioned_model", {"version": 1})

# Publish v2 (same name)
id2 = flowfile_ctx.publish_global("versioned_model", {"version": 2})
...
latest = flowfile_ctx.get_global("versioned_model")
assert latest["version"] == 2, f"Expected version 2, got {latest}"

# Get specific version
v1 = flowfile_ctx.get_global("versioned_model", version=1)
...
flowfile_ctx.publish_global("flow_published_model", {"trained_on": "flow_data"})
...
flowfile_ctx.publish_global("cross_flow_artifact", {"source": "flow_1", "value": 42})
flowfile_ctx.publish_output(df)
'''
...
artifact = flowfile_ctx.get_global("cross_flow_artifact")
...
artifact_id = flowfile_ctx.publish_global("numpy_matrix", arr)
print(f"Published numpy array, id={artifact_id}")

# Retrieve and verify
retrieved = flowfile_ctx.get_global("numpy_matrix")
...
artifact_id = flowfile_ctx.publish_global("polars_df", df)
print(f"Published Polars DataFrame, id={artifact_id}")

# Retrieve and verify
retrieved = flowfile_ctx.get_global("polars_df")
...
artifact_id = flowfile_ctx.publish_global("model_config", config)
print(f"Published nested config, id={artifact_id}")

# Retrieve and verify
retrieved = flowfile_ctx.get_global("model_config")
...
artifact_id = flowfile_ctx.publish_global("custom_model", model)
print(f"Published custom object, id={artifact_id}")

# Retrieve and verify
retrieved = flowfile_ctx.get_global("custom_model")
...
class TestGlobalArtifactsErrorHandling:
    ...
    def test_delete_nonexistent_raises_key_error(
        ...
        code = '''
try:
    flowfile_ctx.delete_global_artifact("nonexistent_artifact_xyz")
    print("ERROR: Should have raised KeyError")
except KeyError as e:
    print(f"Correctly raised KeyError: {e}")
'''
    ...
    def test_get_specific_version_not_found(
        ...
        code = '''
# Publish version 1
flowfile_ctx.publish_global("versioned_test", {"v": 1})

# Try to get version 999 (doesn't exist)
try:
    flowfile_ctx.get_global("versioned_test", version=999)
    print("ERROR: Should have raised KeyError")
except KeyError as e:
