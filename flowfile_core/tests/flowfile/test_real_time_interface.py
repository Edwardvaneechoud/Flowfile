...
PARITY_CASES = [
    ...
    ("type error", '[Amount] + "x"'),
    # Only the collect type-checks .dt, so this shape reaches the strip and the preview by
    # different routes; the shared classifier is what keeps their words identical.
    ("date function on a text column", 'format_date([First], "%Y")'),
]
