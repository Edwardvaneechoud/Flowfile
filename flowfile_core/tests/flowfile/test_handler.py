...
def test_add_flow():
    handler = create_flowfile_handler()
    first_id = handler.add_flow('new_flow', 'flowfile_core/tests/support_files/flows/tmp/new_flow.yaml')
    sleep(1)
    second_id = handler.add_flow('second_flow', 'flowfile_core/tests/support_files/flows/tmp/second_flow.yaml')
