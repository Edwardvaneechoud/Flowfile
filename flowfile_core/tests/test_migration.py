...
class TestOAuthColumnsMigration:
    _NEW_COLUMNS = (
        "oauth_client_id",
    ...
    def test_upgrade_from_031_adds_the_columns(self, tmp_path, monkeypatch):
        from alembic import command
        ...
        db_path = tmp_path / "catalog.db"
        monkeypatch.setenv("FLOWFILE_DB_PATH", str(db_path))
        command.upgrade(_get_alembic_config(), "031")
        assert not set(self._NEW_COLUMNS) & self._columns(db_path, "database_connections")
    ...
    def test_existing_rows_read_back_null(self, tmp_path, monkeypatch):
        """Pre-032 connections carry no OAuth config; never half-migrated."""
        from alembic import command
        ...
        with engine.connect() as conn:
            conn.execute(
                text(
                    "INSERT INTO database_connections (connection_name, database_type, username, user_id) "
                    "VALUES ('legacy_conn_oauth', 'postgresql', 'u', 1)"
                )
            )
            conn.commit()
        engine.dispose()
        ...
        with engine.connect() as conn:
            row = conn.execute(
                text(
                    "SELECT oauth_client_id, oauth_token_endpoint, oauth_client_secret_id, oauth_refresh_token_id "
                    "FROM database_connections WHERE connection_name = 'legacy_conn_oauth'"
                )
            ).one()
        engine.dispose()
    ...
    def test_upgrade_is_guarded_when_a_column_already_exists(self, tmp_path, monkeypatch):
        ...
        engine = create_engine(f"sqlite:///{db_path}")
        with engine.connect() as conn:
            conn.execute(text("ALTER TABLE database_connections ADD COLUMN oauth_client_id VARCHAR"))
            conn.commit()
        engine.dispose()
