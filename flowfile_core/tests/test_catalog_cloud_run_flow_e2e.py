...
Cloud catalog *writes* offload to the worker (``flow_graph._handle_physical_table_write``
forces the worker on a cloud destination, regardless of ``OFFLOAD_TO_WORKER``); the
session-scoped ``flowfile_worker`` fixture in ``conftest.py`` provides it. Cloud
*reads* stay lazy in core (``pl.scan_delta`` with resolved ``storage_options``).
Gated on the shared MinIO mock via ``requires_minio``.
"""
