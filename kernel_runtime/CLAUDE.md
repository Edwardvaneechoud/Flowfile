...
- **Version contract:** image / `pyproject.toml` version (`0.3.0`) evolves independently of the app version (root 0.9.1) and of the runtime API `__version__` (`kernel_runtime/__init__.py`, currently `0.2.2`, surfaced by `/health`). polars (`>=1.8.2,<1.40`) and pyarrow (`^18`) are aligned with core; core's `flavours.py` reads `kernel_runtime/poetry.lock` as the source of truth for baked versions.
