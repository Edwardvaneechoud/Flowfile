...
class SecretSelector(FlowfileInComponent):
    ...
    def set_execution_context(self, user_id: int, accessed_secrets: set, resolver: SecretResolver | None = None):
        """Called by framework before process() runs."""
        self._user_id = user_id
        self._accessed_secrets = accessed_secrets
        self._resolver = resolver

    def _default_resolve(self) -> SecretStr:
        # In-core execution path: no resolver injected, resolve via core's
        # secret manager. Deliberately a lazy import — the SDK itself must
        # stay importable without flowfile_core (worker/dry-run children).
        try:
            from flowfile_core.secret_manager.secret_manager import decrypt_secret, get_encrypted_secret
        except ImportError as e:
            raise SecretResolutionError(
                "No secret resolver is configured for this execution context. "
                "Secrets can only be accessed during node execution."
            ) from e
    ...
    def secret_value(self) -> SecretStr | None:
        ...
        if self._resolver is not None:
            decrypted = self._resolver.resolve(self.value)
        else:
            decrypted = self._default_resolve()
