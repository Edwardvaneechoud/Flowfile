...
def scd2_into_delta(
    ...
    *,
    ...
    """Apply an SCD2 batch to a Delta table in a single atomic commit.

    A missing target is an initial load (every row stamped current, ``mode="error"`` create),
    partitioned by the is-current column with any *partition_by* columns nested above it — closed
    rows then land in files later merges never rewrite, and current-state scans prune to one
    partition per user-partition. ``partition_on_current=False`` is the deliberate opt-out. An
    existing target must already be SCD2-shaped: rows whose *compare_columns* changed are end-dated
    (``valid_to = valid_from_iso``, ``is_current = false``) and re-inserted as a new version, new
    keys are inserted, and with *full_snapshot* current keys absent from *df* are end-dated too.
    Close and insert ride one ``MERGE``, so a reader never observes a torn state and an effective
    write produces exactly one Delta version. Nothing to do ⇒ ``Scd2Result(skipped=True)`` with zero
    commits, which is what makes re-running an identical batch a true no-op.

    *valid_from_iso* is one processing instant for the whole write, generated by the caller — the
    surrogate key is a function of (business key, valid_from), so re-deriving the clock here would
    mint different keys on the local and worker paths of the same write. Validity is half-open:
    ``[valid_from, valid_to)``. Timestamps are ``Datetime("us", "UTC")``, which round-trips Delta
    byte-exactly (a naive datetime would land as ``timestamp_ntz``).

    Empty *compare_columns* means every non-key column; any business key named there is dropped
    (a matched row's keys are equal by definition). Only ``CommitFailedError`` is retried, and the
    retry re-reads and re-classifies, so a decision made against a superseded snapshot is never
    re-committed. Re-classification alone is not enough, though — the frozen instant may now be
    behind the winner's, so every effective write is also checked against the table's newest
    version and raises rather than emitting an inverted validity interval.
    *storage_options* is forwarded verbatim: ``None`` is the local filesystem, an empty dict means
...
def _scd2_guard_clock(
    ...
    """Refuse to write with an instant that is not strictly newer than the table's current slice.

    The write end-dates current rows with *ts*, so an instant at or behind the newest ``valid_from``
    would emit ``valid_to <= valid_from`` — a version invisible to every ``active_at`` read of the
    span it actually covered. Two ways in: clock skew between machines sharing one catalog, and a
    retry after a commit conflict whose winner stamped a later instant than ours. Failing here keeps
    the surrogate key a pure function of the caller's instant; the next run gets a fresh clock.
    """
    import polars as pl_
    ...
    newest = (
        pl_.scan_delta(output_path, **scan_kwargs)
        .filter(pl_.col(is_current_column))
        .select(pl_.col(valid_from_column).max())
        .collect()
        .item()
    )
    if newest is None:
...
def _scd2_once(
    ...
    *,
    ...
    if n_changed == 0 and n_new == 0 and not needs_snapshot_close:
        return Scd2Result(skipped=True)
    if current_rows:
        _scd2_guard_clock(output_path, ts, valid_from_column, is_current_column, storage_options)
