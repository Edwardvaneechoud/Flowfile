...
| `test_e2e` | `npm run build:web`; starts `poetry run flowfile_core` + `flowfile_worker` + `npm run preview:web` (:4173); runs `tests/web-flow.spec.ts` + `tests/csp.spec.ts`, then stops the listeners. On macOS/Linux it exits with Playwright's status; the Windows branch keeps `\|\| true` (failures swallowed) |
| `test_e2e_dev` | Same, but uses `npm run dev:web` (:8080) instead of the preview server |
| `test_e2e_cloud` | macOS/Linux + Docker only: `start_minio` + `seed_cloud_e2e`, then an isolated core/worker/vite-preview on **free** ports (own DB, storage, secure store, `HOME`, empty working dirs, MinIO-only AWS profile under `env -i`; servers start with `poetry -P <repo> run` on Poetry 2 or `poetry -C <repo> run` on 1.x — Poetry 2's `-C` changes into the repo, which would defeat the empty-cwd check — with `VIRTUAL_ENV` pinning the venv the isolated `HOME` would otherwise hide), runs `pytest tests/cloud_e2e` and `npm run test:cloud`; a trap kills only the PIDs it started and deletes its `cloud-e2e-<run id>/` MinIO prefix; fails if a suite fails or a server wrote into its working dir |
| `stop_servers` | Stops only the processes **listening** on 63578/63579/8080/4173 (found with `lsof`, else `ss`; prints a warning when neither exists). On Windows it is still `taskkill /F /IM python.exe` and `node.exe` — kills **every** Python/Node process, not just this project's |
...
6. **On Windows, `make test_e2e` still swallows Playwright failures with `|| true`** — a red Playwright run will NOT fail the target there (macOS/Linux now exit with Playwright's status). `stop_servers` on Windows kills **all** `python.exe`/`node.exe` processes, not just this project's. Never dry-run a target whose recipe line contains `$(MAKE)`: `make -n` still executes those lines.
