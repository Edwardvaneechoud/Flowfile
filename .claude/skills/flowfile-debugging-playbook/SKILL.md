...
| A scheduled flow's log is missing from `$FLOWFILE_STORAGE_DIR/logs` | Historical: the run-log path used to be hardcoded to `~/.flowfile/logs`, ignoring `FLOWFILE_STORAGE_DIR` and docker mode | All three sites now route through `shared/run_logs.py::run_log_path`. Logs written by older versions are still under the real home dir | Fixed. If a path looks wrong, check nothing bypasses `run_log_path` |
| A run's log is gone after restarting core, even though the run definitely happened recently | Historical: `clear_all_flow_logs()` wiped every `*.log` (run logs included) at shutdown, and `cleanup_directories()` swept the logs dir at a hardcoded 168h | Logs now expire only by age (`FLOWFILE_RUN_LOG_RETENTION_DAYS`, default 30d). Cross-check the durable record: `sqlite3 ~/.flowfile/database/flowfile_catalog.db 'select id,flow_name,started_at,ended_at,success from flow_runs order by id desc limit 10;'` | Fixed. Do not re-add a logs sweep to `cleanup_directories()` — it silently overrides the env var |
...
| Log | Location | Notes |
|---|---|---|
| Per-flow execution log | `<storage base>/logs/flow_<flow_id>.log` | Truncated at every run start, so it holds only the latest run; expires by age (`FLOWFILE_RUN_LOG_RETENTION_DAYS`, default 30d) |
| Scheduled-run subprocess output | `<storage base>/logs/scheduled_run_<run_id>.log` | Resolved via `shared/run_logs.py::run_log_path` (honors `FLOWFILE_STORAGE_DIR` / docker); one file per run, kept until it ages out |
