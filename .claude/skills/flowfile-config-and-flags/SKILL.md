...
- `FLOWFILE_WORKER_HOST` (conftest.py:53, default `0.0.0.0`), `FLOWFILE_STARTUP_TIMEOUT` (`:56`, 30s), `FLOWFILE_SHUTDOWN_TIMEOUT` (`:58`, 15s), `SKIP_WORKER_TESTS` (`:226`, `== "1"`), `FLOWFILE_TEST_REUSE_WORKER` (`conftest.py::_claim_worker_port`, `== "1"`: reuse a worker already on the default 63579 instead of moving the session to a free port; an explicit `FLOWFILE_WORKER_PORT` is always used as given) — test-only. **`FLOWFILE_WORKER_HOST` is NOT read by package source** (which uses `WORKER_HOST`) — near-identical name, different variable, easy to set the wrong one.
