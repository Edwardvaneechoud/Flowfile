...
- **WASM:** JSON read unsupported in browser (`flowfile_wasm/src/stores/flow-store.ts:2026`), Arrow files unsupported (`src/utils/remote-file.ts:33`, `ReadFileSettings.vue:260`). The formula-node publish gate from user memory is **resolved**: `useFormulaTranslation.ts:9` (`EXPR_TRANSFORMER_PACKAGE`, the single src/ pin) pins `polars-expr-transformer==0.6.0` and `src/pyodide/engine/nodes_formula.py` exists.
