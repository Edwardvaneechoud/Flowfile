...
- `tests/` — package-root test dir (sibling of the inner package); `tests/test_cron_schedules.py` holds cron evaluation tests, `tests/test_table_trigger_schedules.py` the table-trigger watermark and orphan-reaper tests.
...
- **Four schedule types**, dispatched by `_tick` in order: `interval`, `cron`, `table_trigger`, `table_set_trigger` (each `_process_*_schedules` returns a launch count). Filter is always `enabled.is_(True)` plus the matching `schedule_type`. Before the processors (and inside the leader lock, so two instances can't race on SQLite) `_tick` calls `shared.run_completion.reap_orphaned_runs()` in a try/except: a spawned run whose child died never writes `ended_at`, and the row would otherwise block that flow forever behind the active-run guard. Alongside it, still inside the lock, `_tick` runs `shared.run_logs.cleanup_old_logs()` — throttled to once per `LOG_SWEEP_INTERVAL` (3600s) via a `time.monotonic()` stamp, since the 30s tick would otherwise glob the logs dir 2880×/day — so a standalone scheduler expires logs even when core startup never runs. Still inside the lock, `_tick` then calls `shared.notifications.processor.process_pending_notifications()` in a try/except — it evaluates newly-ended runs into the notification outbox and drains that outbox over webhooks.
...
- **Double-launch guard:** `_maybe_launch` skips if `_has_active_run` finds a `FlowRun` with `ended_at IS NULL` matching **either** `registration_id` or `flow_uuid` (the uuid arm mirrors core's queries; the id arm still counts pre-fix rows with a NULL uuid). It creates the `FlowRun` row (`run_type="scheduled"`, `flow_uuid`, `schedule_id`, later `pid`) and sets `last_triggered_at` **before** spawning, so the run shows active immediately; spawn failure marks the run `ended_at`/`success=False`. It returns a `LaunchOutcome` (`LAUNCHED`/`SKIPPED`/`FAILED`) — the trigger paths need `SKIPPED` (guard declined) and `FAILED` (missing registration / broken spawn) to move the watermark differently.
- **table_trigger is a poll safety-net:** the primary/fast trigger is a push path on the catalog-write side (`CatalogService._fire_table_trigger_schedules`, reached via `overwrite_table_data`). This package's poll path compares `CatalogTable.updated_at` against the schedule's `last_trigger_table_updated_at` as a fallback. `table_set_trigger` requires ≥2 linked tables (via `ScheduleTriggerTable`), all advanced past the same watermark column.
- **Arm, don't replay:** a NULL `last_trigger_table_updated_at` means "never observed", not "changed" — `CatalogTable.updated_at` is never NULL, so treating NULL as a change fired a phantom run on the first tick after creation. Both trigger paths arm such a schedule with the table's current `updated_at` (max across the set) and launch nothing. Watermarks always carry an **observed** `updated_at`, never `_utcnow()` (which would swallow a write landing between the read and the stamp), and advance on `LAUNCHED`/`FAILED` but **not** on `SKIPPED`, so a trigger blocked by an active run re-fires once that run ends. Core arms the watermark up front too (`ScheduleService._arm_trigger`, on create and on disabled→enabled).
