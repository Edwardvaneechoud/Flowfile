...
- **`resolve_virtual_table` has two sanctioned targets** (`funcs.py`): the default `virtual_results` (worker-private `catalog_virtual_results/`, idempotent cache on `(table_id, source_versions_hash)` — the preview/SQL contract) and `kernel_shared` (`storage.shared_virtual_results_directory`, kernel-readable catalog-view snapshots) which **always rebuilds** — serialised-plan hashes don't change when referenced Delta data does, so an exists() cache would serve stale data — and prunes superseded same-table snapshots only when older than `_SUPERSEDED_MIN_AGE_S`.
- **`resolve_virtual_table`'s filename stem is `cache_key or f"fvt-{table_id}"`.** `cache_key` is how non-catalog callers (flow-node visualizations, `node-{flow_id}-{node_id}`) reuse the same materialiser; it is charset-restricted (`^[A-Za-z0-9_-]{1,64}$`) at the model because the worker builds a path from it and must not trust core. `cache_key` callers fold a per-run token into `source_versions_hash` rather than being content-addressed, so their snapshots are swept on every request — otherwise every flow run would leave a full-result IPC file behind.
