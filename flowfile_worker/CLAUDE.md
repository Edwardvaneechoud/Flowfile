...
- **`resolve_virtual_table` has two sanctioned targets** (`funcs.py`): the default `virtual_results` (worker-private `catalog_virtual_results/`, idempotent cache on `(table_id, source_versions_hash)` — the preview/SQL contract) and `kernel_shared` (`storage.shared_virtual_results_directory`, kernel-readable catalog-view snapshots) which **always rebuilds** — serialised-plan hashes don't change when referenced Delta data does, so an exists() cache would serve stale data — and prunes superseded same-table snapshots only when older than `_SUPERSEDED_MIN_AGE_S`. The child collects with `engine="streaming"` and writes `compression="lz4"` — `write_ipc` defaults to **uncompressed**, and dictionary-encoded Parquet strings expand many-fold when written out raw (a 294 MB source produced a 4.9 GB snapshot before this).
