...
class Selector:
    ...
    def count(self) -> 'Expr':
        ...
        from flowfile_core.flowfile.flowfile_frame.expr import Expr
        # Represents pl.count(selector) - adjust if pl.col(selector).count() is needed
...
class CompoundSelector(Selector):
    ...
    def __init__(self, left: Selector, right: Selector, operation: str):
        ...
        super().__init__()
    def _get_repr_str(self) -> str: # Use base method name
...
# --- Specific Selectors (Corrected Syntax) ---
...
class DtypeSelector(Selector):
    ...
    def _get_repr_str(self) -> str:
        ...
        for dt in self.dtypes:
            dt_repr = repr(dt);
            if dt_repr.startswith("DataType"): dt_repr = str(dt).capitalize()
...
class PatternSelector(Selector):
    def __init__(self, pattern: str):
        self.pattern = pattern; super().__init__()
...
# --- Factory Functions (Unchanged, assuming renames like float_ are intended) ---
