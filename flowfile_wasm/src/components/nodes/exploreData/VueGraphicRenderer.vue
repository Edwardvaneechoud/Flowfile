...
/**
 * Read-only Graphic Walker chart renderer for dashboard tiles. Ported from
 * flowfile_frontend's VueGraphicRenderer, but client-side: it renders from an
 * in-memory ``data`` array (Pyodide-materialised rows) instead of a worker
 * ``computation`` callback. GraphicRenderer aggregates the rows in-browser.
 */
...
// GraphicRenderer's own ResizeObserver mis-measures height (its outer wrapper
// is missing h-full), so we measure the tile ourselves and pass the result as
// `overrideSize: { mode: 'fixed', ... }` — the library treats it as a hard
// replacement for layout.size.
...
// Small props can be deep-watched cheaply; `data` may hold up to 100k rows, so
// watch it by reference only (filtered arrays arrive as a fresh reference).
watch(
