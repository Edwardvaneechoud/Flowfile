...
import { ISegmentKey } from '@kanaries/graphic-walker'
...
interface VueGWProps {
   ...
   *
   * If both ``data`` and ``computation`` are provided, ``computation`` wins
   * and ``data`` is ignored — GW will not need it. The WASM build normally
   * uses the local-data path (Pyodide materializes rows up-front), but the
...
const dummyComputation = async (): Promise<IRow[]> => {
  console.warn(
    '[VueGW] Dummy computation function called. This should not happen when providing local data.'
  )
  return []
}
...
onMounted(async () => {
  ...
  try {
    // Dynamic imports - only loaded when component mounts. This keeps
    // React + Graphic Walker out of the initial bundle for users who never
    // open an explore_data node.
    ...
    // Set the default tab if specified. The store is populated asynchronously
    // after GW mounts its React tree, so poll for it briefly.
    if (props.defaultTab) {
 ...
 * isn't ready (no fields, render error, or unmounted).
 */
...
.gw-wrapper > div {
  ...
  /* React mount must be a deterministic flex item, not `height: 100%`.
     ...
     every ancestor — under fullscreen / dialog flex layouts that fails on
     ...
     visible) until it hits the parent's clip. With `flex: 1 1 0` plus
     `overflow: hidden` here, GW's chart area is forced into a definite
     box, engaging its native `.overflow-auto` for chart scroll. */
  flex: 1 1 0;
  display: flex;
  flex-direction: column;
  min-width: 0;
