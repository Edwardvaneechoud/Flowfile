<template>
  <div class="settings-form">
    ...
    <div v-if="!availableDatasets.length" class="info-message">
      <p>No catalog tables yet. Open the <strong>Catalog</strong> and use <strong>Upload table</strong> to add a CSV, then select it here.</p>
    </div>

    <div v-if="selectedDataset && !dataLoaded" class="warning-message">
      Table "{{ selectedDataset }}" isn't in the Catalog anymore. Re-upload it in the Catalog.
    </div>
...
import type { NodeReadFromCatalogSettings, NodeSettings } from '../../types'
...
const availableDatasets = computed(() => flowStore.getCatalogDatasetNames())
...
onMounted(() => {
  const s = props.settings as NodeReadFromCatalogSettings
...
watch(() => props.settings, (newSettings) => {
  const s = newSettings as NodeReadFromCatalogSettings
  selectedDataset.value = s.dataset_name || ''
}, { deep: true })

// Refresh when catalog datasets change (e.g. uploaded/removed in the Catalog).
watch(() => flowStore.catalogDatasets, () => {
  if (selectedDataset.value) {
    loadDataset(selectedDataset.value)
  }
}, { deep: true })
...
function handleDatasetChange(name: string) {
  ...
  const currentSettings = props.settings as NodeReadFromCatalogSettings
  const updated: NodeReadFromCatalogSettings = {
    ...currentSettings,
    dataset_name: name,
    is_setup: !!name
  }
