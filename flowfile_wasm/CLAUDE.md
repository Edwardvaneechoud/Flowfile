...
`tests/unit/plain-python-parity.test.ts` shells out to a CPython with Polars (env var `FLOWFILE_TEST_PYTHON`, else `python3`/`python`, else a `flowfile-*` Poetry venv). Without one it reports **skipped**, not passed — if you see `37 skipped`, nothing was actually compared.
