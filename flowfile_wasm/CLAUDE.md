...
- `src/composables/useCodeGeneration.ts` — generates standalone Python/Polars scripts from a flow (CodeGenerator modal). Exports `FlowToPolarsConverter`; its members are `protected`, not `private`, because the plain-Python flavour subclasses it.
