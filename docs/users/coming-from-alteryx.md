...
- **`.yxdb` files written by the AMP engine (E2) are read through `sigilyx`**, a second optional extra (`pip install 'flowfile[alteryx-amp]'`); original-engine files keep the existing reader. AMP support is **experimental**: a file whose records do not decode is refused rather than written with blanks, so you get an error, never a table of empty rows. Alteryx's own three AMP sample files convert (11, 140 and 3598 records, no blanks); a mixed fixed-width AMP file has been seen returning blanks for 9 918 of 10 000 records, which is exactly what the refusal catches. If yours is refused, export it to CSV from Alteryx, or write it with *Use AMP Engine* off.
