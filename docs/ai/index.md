...
No hosted Flowfile service — bring your own key (Anthropic, OpenAI, Google, Groq, OpenRouter, or a local Ollama), or run the built-in [on-device model](providers.md#on-device-model-no-key-required) without a key. See [Provider Setup](providers.md).
