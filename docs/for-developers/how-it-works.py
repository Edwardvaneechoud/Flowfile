...
def test_another_implementation():
    ...
    # flow_id=9090 node_id=5 cache_results=False pos_x=200.0 pos_y=200.0 is_setup=True description='Aggregate after grouping by "region"' user_id=None is_flow_output=False depending_on_id=4 groupby_input=GroupByInput(agg_cols=[AggColl(old_name='region', agg='groupby', new_name='region', output_type=None), AggColl(old_name='total', agg='sum', new_name='total_revenue', output_type=None), AggColl(old_name='total', agg='mean', new_name='total_quantity', output_type='Float64')])
    df_5 = df_3.collect().group_by([(ff.col("region").str.to_uppercase() + ff.lit("test"))]).agg(
                         ff.col("total").sum().alias("total_revenue"),
                         ff.col("total").mean().alias("total_quantity"),
                         )
    df_5.collect()