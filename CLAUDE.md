...
- **Database & migrations**: one SQLite catalog DB (`flowfile_catalog.db`) shared by core, scheduler, and worker. URL resolved in `shared/storage_config.get_database_url()`. Schema changes use **Alembic** (`flowfile_core/flowfile_core/alembic/versions/NNN_*.py`, currently 001–020), run automatically at core startup via `flowfile_core/database/migration.py`. Add a migration when changing `flowfile_core/database/models.py`; keep the numeric `NNN_` prefix sequence.
