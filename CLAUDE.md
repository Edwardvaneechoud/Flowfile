...
- **Conditional execution** (the `gate` node + `flowfile_core/flowfile/util/skip_rules.py`): which nodes run is decided by a **local trigger rule applied in topological order**, not by a transitive closure — ALL by default (any error-ish input error-skips; any deliberately-skipped input deliberately-skips), ANY for `union` only (runs if ≥1 input survived and none failed). Gate conditions are a plan-time **flow-parameter rule** (operator + value, no invert — use the complementary operator) or a run-time **formula** (a flowfile-formula row predicate: open iff ≥1 row matches, checked against the optional control input else the gate's own data input). A closed gate succeeds but its downstream is **deliberately skipped**: green `NodeResult(skipped=True)` rows, progress still reaching N/N, and source post-run callbacks (Kafka offset commits) still firing. Failure-driven skips are unchanged and still block everything, unions included. Cross-package reach: schema-stable empty-frame substitution for gated-off union inputs (`flow_node.py`), real `if`-blocks in all three code exports — Polars, FlowFrame, and project (`code_generator.py`; the ff export wraps empty stand-ins as `ff.FlowFrame` and probes formula gates via `.data`), a `skipped` state on the canvas/run report (frontend), and `available: false` in WASM.
