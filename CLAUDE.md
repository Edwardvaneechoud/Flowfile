...
- **Database & migrations**: one SQLite catalog DB (`flowfile_catalog.db`) shared by core, scheduler, and worker. URL resolved in `shared/storage_config.get_database_url()`. Schema changes use **Alembic** (`flowfile_core/flowfile_core/alembic/versions/NNN_*.py`; `ls` the dir for the current head), run automatically at core startup via `flowfile_core/database/migration.py`. Add a migration when changing `flowfile_core/database/models.py`; keep the numeric `NNN_` prefix sequence. Before applying pending migrations (or an unknown-revision re-stamp), core snapshots the DB into a sibling `db_backups/` dir via `database/backup.py` (best-effort, sqlite backup API; retention `FLOWFILE_DB_BACKUP_KEEP`, default 10, `0` disables). The same module also snapshots on demand — `backup.create_snapshot(db_path, "manual" | "pre_update")` and `backup.list_snapshots`, exposed as the admin `GET`/`POST /system/db_backups` routes (`routes/system_backups.py`) behind Settings → Preferences → Backups and the desktop update prompt; listing is read-only, restore is a documented manual file copy.
...
- **Browser share links** (`flowfile_core/flowfile_core/flowfile/share/` + `GET /editor/share_link`): core mints serverless share URLs for the in-browser WASM editor — `https://demo.flowfile.org/designer#flow=<base64url(deflate-raw(JSON))>`, envelope `{v:1, flow}` matching `flowfile_wasm/src/utils/share-link.ts` (data lives in the fragment; nothing reaches a server). The transform serializes the **live in-memory** graph (never the save/open path), always emits an explicit `connections` array (WASM's implicit edge derivation hardcodes `output-0`), and **demotes WASM-unsupported or settings-incompatible nodes to placeholders**: sentinel type `<core_type>__unsupported` + a `{is_placeholder, original_type, reason, label, inputs, outputs}` stub — settings are stripped, so connection names/paths/credentials and executable code (`polars_code`, advanced filters — exec'd during WASM schema propagation) never travel; placeholders carry only the user-typed description (auto-generated ones embed settings). One demotion is pre-empted rather than accepted: an advanced filter that is exactly `[col] <op> literal` is rewritten into the equivalent **basic** filter before the compatibility check (`share/filter_translation.py`), so `[quantity] > 7` travels as runnable settings with no expression in the payload; the carve-outs (fractional bounds, boolean/null literals, function calls, multi-condition and column-to-column comparisons) stay placeholders because WASM converts a basic value by the column's dtype while core lits it as written. **Expressions themselves stay demoted** even though WASM now runs advanced filters through the same `simple_function_to_expr` core uses: that parser executes Python for a crafted formula (polars-expr-transformer ≤0.6.0, the latest as of 2026-09 — `standardize_quotes` requotes `'a"b'` unescaped and `Classifier.get_pl_func` `eval`s the result), pinned by a strict xfail in `flowfile_wasm/tests/python/test_build_helpers.py`. The **formula node ships its expression today** and is the same sink, so raising that pin is a prerequisite for trusting either. The supported-node truth is **generated from flowfile_wasm** (`tools/generate_wasm_node_manifest.py` parses `src/config/nodeCatalog.ts` → `flowfile/share/wasm_node_support.json`, shipped as package data like the kernel manifest; `make check_share_data` gates drift + packaging; loader fails **closed** — no manifest ⇒ everything placeholder). WASM renders placeholders locked, blocks their downstream (green-less `blocked` state, not failures), auto-runs the runnable subgraph on link open, and defensively refuses placeholder stubs at every exec sink (`engine/validation.py::refuse_placeholder`). The wasm↔core parity suites run for real in CI via the `core-parity` job in `flowfile-wasm-build.yml` (`FLOWFILE_REQUIRE_TEST_PYTHON` turns their silent skip into a failure). Sender UI: "Share link" in the designer header (`ShareLinkDialog.vue`, distinct from group-sharing's "Share").
- **Catalog change tracking (CDC)** (`shared/delta_utils.py` + `flowfile_core/flowfile/catalog_cdc.py`): incremental reads off a catalog table's **Delta change data feed** (`delta.enableChangeDataFeed`). Enablement is **per table, default off, enable-only** and mirrored onto `CatalogTable.cdc_enabled`/`cdc_enabled_version` — the enablement commit is the **cursor floor**, since reads below it are inconsistent (error at v0, synthesized rows at v1); turned on by the writer's `track_changes` (create-path `configuration=`, else an alter commit just before the write so that write is the first tracked one, both via `flow_graph._ensure_catalog_cdc_enabled` / the service's `enable_cdc`) or `POST /catalog/tables/{id}/cdc/enable`. The reader is **strict**: `NodeCatalogReader.cdc_mode != "off"` on an untracked table is an actionable run-time error, never a fallback to delta-rs's undocumented synthesis; `scan_delta_changes` wraps `DeltaTable.load_cdf(allow_out_of_range=True)` in a `register_io_source` plugin so the LazyFrame **serializes to the worker unchanged** (core still never collects) and an at-head cursor yields an empty frame with full schema. Cursor key = `(table_id, consumer_key)` with `consumer_key = "name:<x>"` (global to the table, deliberately shareable) else `"flow:<flow_uuid>:node:<node_id>"` — neither resolvable ⇒ hard error; unit is the Delta **commit version** (`since_timestamp` is resolved to a version first, so the floor clamp applies to it too; both `cdc_from_version` and `cdc_from_timestamp` accept a whole-field `${param}` ref that the run-time resolver substitutes before `_read_changes` reads them), head pinned **inside `_func`** at execution time (never at wiring — the designer keeps one FlowGraph across runs) and folded into `_catalog_reader_source_fingerprint`. Commits are **at-least-once**: the cursor advances only via `node._on_flow_complete` from `_run_post_execution_callbacks`, so previews, single-node runs and cancels never advance it and downstream writers should upsert, not append. Migration **032** adds the two columns + `catalog_cdc_cursors` (unique `uq_cdc_cursor_table_consumer`); `delete_table` purges cursors and a cursor whose stamped `table_path` no longer matches is treated as absent (SQLite rowid reuse). Routes `GET /catalog/tables/{id}/cdc`, `POST .../cdc/enable`, `POST .../cdc/cursors/reset`, `DELETE .../cdc/cursors/{consumer_key}`, plus a **409 `CDC_CURSORS_AT_RISK`** guard on the existing vacuum route (overridable with `force=true`; overwrite-window feeds are reconstructed from tombstoned files and a vacuum breaks them, merge-produced `_change_data/` survives). Excluded: virtual, legacy-parquet, SQL-mode and SCD2 tables, pinned `delta_version`, `track_changes` with `write_mode in ("overwrite", "virtual", "scd2")`, and WASM/share-links (catalog readers and writers are already `_ALWAYS_PLACEHOLDER` in `share/compatibility.py`, so no CDC settings travel).
- **Cloud Delta merge modes + change feed** (`flow_graph.py::_write_cloud_delta` / `add_cloud_storage_reader` + `routes/cloud_delta.py`): the `cloud_storage_writer`/`cloud_storage_reader` nodes reuse the catalog's Delta primitives on a **bare object-storage path** (S3 + ADLS; `az://` → `abfss://` via `normalize_delta_path` at the call sites, never inside `delta_utils`). Writer: `write_mode` gains `error`/`upsert`/`update`/`delete` plus `merge_keys` and `track_changes` (Delta-only; `CloudStorageWriteSettings` mirrors the catalog validator, `track_changes` rejected with `overwrite`, and the worker's hand-copied `WriteSettings` must match). Merge modes or `track_changes` dispatch the catalog's generic `merge_delta`/`write_delta` worker ops with an owner-encrypted `storage_payload` (local execution merges in-process — the sanctioned local collect); plain overwrite/append/error keep the streaming `sink_delta` path, so an untracked append gets no schema evolution. `enable_cdf=True` on `shared/delta_utils.write_delta`/`merge_into_delta` means **ensure enabled** (create with the property, else a `SET TBLPROPERTIES` commit *before* the write, so that write lands above the floor) — a no-op for the catalog, which enables first. Reader: `cdc_mode` `off|since_version|since_timestamp` via the shared `schemas/change_feed.py::ChangeFeedReadSettings` mixin (`NodeCatalogReader` widens it with `since_last_run`); **no cursors for cloud paths** (`scan_delta(changes_since="last_run")` raises; the moment advances through a `${param}`); the window comes from the shared `catalog_cdc.resolve_change_window`, and with no `CatalogTable.cdc_enabled_version` the **floor is derived from the Delta log** on every run (`delta_utils.get_change_data_feed_floor`: newest `SET TBLPROPERTIES` that turned CDF on, else the oldest retained version; `None` ⇒ actionable untracked-table error). Head is pinned inside `_func`, a `schema_callback` predicts log-metadata columns + `_CDF_COLUMN_DTYPES`, and `_refresh_catalog_source_freshness` also probes CDC cloud readers (`_cloud_change_read_fingerprint`). Time travel (`delta_version`) and change reads are mutually exclusive. Routes `POST /cloud_storage/delta/info`, `/cdc/enable`, `/history` (JWT, no trailing slash, connection resolved for the caller via `storage_browser._resolve_connection`, metadata only, provider error text scrubbed of connection values). **GCS is refused** for merge modes, `track_changes`, change reads and the routes (delta-rs takes no gcsfs-style options). Both nodes resolve their connection through `flow_graph._resolve_cloud_node_connection`, which first runs `shared/cloud_storage/utils.py::validate_cloud_resource_path` inside `_func`: an empty or schemeless relative path is a `ValueError` before any I/O (polars would otherwise use the server's working directory; worker `WriteSettings` re-checks it). A node with no connection takes its storage type from the path scheme (`uri.py::storage_type_for_uri`, S3 when schemeless; `az://`/`abfss://`/`gs://` fall back to `env_vars`, since `aws-cli` is S3-only). SCD2 stays catalog-only; no migration; both cloud nodes are `locked` share-link placeholders, so no CDC settings travel.
- **Python API native node classes** (`flowfile_frame/{native,gate,parameters,run_flow,custom_node,custom_nodes,python_script,sql_query,enums}.py`, re-exported as `fl.*` by `flowfile/flowfile/__init__.py`): canvas node types with no fluent method (`gate`, `run_flow`, `flow_input`/`flow_output`, custom nodes, `python_script`, and any built-in type via generic `fl.Node`; `fl.sql` places a SQL Query node over named frames) are placed from Python on the `NativeNode` base in canvas order (merge input graphs → free id → promise → wire → `add_<type>`), so the result opens and round-trips in the designer; every error is `NativeNodeError` (a `ValueError`). Nodes whose output only exists once the flow runs (subflows, kernel code, external sources, and writers/model nodes below them) yield **deferred frames**: seeded with typed zero-row outputs (`deferred_until_run`) so building never executes them, while `collect()` on one runs only its lineage (`run_graph(node_ids=...)`; Docker when that lineage holds a kernel node) and reads the node's real, gate-aware output. `fl.register_flow` / `RunFlow(graph, name=...)` write a YAML + catalog row at build time. Invariants: `flowfile_frame/CLAUDE.md`; user docs: `docs/users/python-api/reference/native-nodes.md`.
...
| `make stop_servers` | Stop only the processes listening on 63578 / 63579 / 8080 / 4173 (found with `lsof`, else `ss`); Windows still `taskkill`s every `python.exe` / `node.exe` |
| `make test_e2e_cloud` | Cloud storage E2E (macOS/Linux + Docker): seeds MinIO, runs an isolated core/worker/vite-preview on free ports from empty working dirs with a MinIO-only AWS profile, then `tests/cloud_e2e` and `cloud-storage-flow.spec.ts`; kills only its own PIDs, fails if a suite fails or a server wrote into its working dir |
...
# Cloud storage E2E: spawns its own core + worker on free ports, drives them over HTTP against MinIO (Docker)
poetry run pytest tests/cloud_e2e -m cloud_e2e
```
...
| `cloud_e2e` | Real core + worker processes driven over HTTP against MinIO (Docker; skips locally without it, fails when `CI` is set) |
...
# Cloud storage spec: needs a seeded MinIO (`poetry run seed_cloud_e2e` from the repo root) and an
# explicit API_URL/TEST_URL (it skips otherwise); E2E_AWS_PROFILE_CONFIGURED=1 also runs "No connection"
npm run test:cloud
```
...
make test_e2e_cloud    # MinIO + an isolated stack on free ports; tests/cloud_e2e + cloud-storage-flow.spec.ts
```
...
> worker). On macOS/Linux it exits with Playwright's status (Windows still ignores
> it). `poetry run seed_cloud_e2e` idempotently writes
> `s3://flowfile-test/cloud-e2e/source.parquet`, the source the cloud spec reads.
> Tauri-shell E2E tests via `tauri-driver` are a follow-up;
...
|----------|---------|-------------|
| `test.yaml` | Push/PR to `main` (no path filter) | **Primary CI**: backend tests (Linux+macOS matrix; the Linux jobs also run `tests/cloud_e2e` against MinIO), Windows job, kernel tests, `check-stubs`, web tests, docs build, Codecov upload |
| `e2e-tests.yml` | Push/PR to `main` (`flowfile_frontend/**`, `flowfile_core/**`, `flowfile_worker/**`, `shared/**`, `test_utils/s3/**`) | Build frontend, start and seed MinIO, start core + worker from empty working dirs with a MinIO-only AWS profile (`AWS_ENDPOINT_URL` exported, so nothing reaches AWS), run Playwright web E2E incl. `cloud-storage-flow.spec.ts`; fails if a server wrote into its working dir |
...
- `FLOWFILE_DB_BACKUP_KEEP` (default `10`) — how many catalog-DB snapshots to keep in `db_backups/` next to the DB file (pre-migration, pre-update and manual snapshots share the budget); `0` disables snapshotting.
...
- `FLOWFILE_CONSOLE_SOURCE` (default off; `1`/`true`/`yes`/`on`, any case, enables) — read once when `flowfile_frame` is imported, it forces on the audit hook in `flowfile_frame/_console_source.py` that records console-defined function source (for `@ff.python_script`), which otherwise installs only in an interactive console (`sys.flags.interactive`, `sys.ps1` or `pydevconsole`).
...
- Polars is pinned `>=1.17.0, !=1.43.0, !=1.43.1, <1.44` (one cross-platform pin — the old Windows `<=1.25.2` ceiling was removed; the 1.17 floor is `join(maintain_order=)`, which the SCD2 writer's output relies on); polars 1.43.0/1.43.1 deadlock `SQLContext.execute` over `scan_delta` frames (hangs every catalog SQL reader/view — fixed in 1.43.2; verify the 5-line repro passes before raising the ceiling), and any bump must be coordinated with the version-coupled `polars-*` plugin packages and `kernel_runtime`
- Tests and test_utils are excluded from Ruff linting (except specific per-file rules)
- The `kernel`, `docker_integration`, `kafka`, and `cloud_e2e` pytest markers all require Docker
