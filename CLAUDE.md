...
- **Python API native node classes** (`flowfile_frame/{native,gate,parameters,run_flow,custom_node,python_script,enums}.py`, re-exported as `fl.*` by `flowfile/flowfile/__init__.py`): canvas node types with no fluent method (`gate`, `run_flow`, `flow_input`/`flow_output`, custom nodes, `python_script`, and any built-in type via generic `fl.Node`) are placed from Python on the `NativeNode` base in canvas order (merge input graphs → free id → promise → wire → `add_<type>`), so the result opens and round-trips in the designer; every error is `NativeNodeError` (a `ValueError`). Nodes whose output only exists once the flow runs (subflows, kernel code, external sources, and writers/model nodes below them) yield **deferred frames**: seeded with typed zero-row outputs (`deferred_until_run`) so building never executes them, while `collect()` on one runs the whole `run_graph()` (every writer, Docker when a kernel node is present) and reads the node's real, gate-aware output. `fl.register_flow` / `RunFlow(graph, name=...)` write a YAML + catalog row at build time. Invariants: `flowfile_frame/CLAUDE.md`; user docs: `docs/users/python-api/reference/native-nodes.md`.
