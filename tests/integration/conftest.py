...
log = logging.getLogger("flowfile.e2e")
...
def _wait_for_service(url: str, label: str, path: str = "/health/status", timeout: float = 120) -> bool:
        ...
        try:
            with httpx.Client(timeout=5.0) as client:
                ...
                if resp.status_code == 200:
                    log.info("[%s] healthy after %d attempts", label, attempt)
                    return True
...
def compose_services():
    ...
    log.info("Pre-flight: checking Docker availability...")
    try:
    ...
    log.info("Pre-flight: checking ports 63578, 63579...")
    ...
    # -- Step 2: build images --
    log.info("Building core + worker images (this may take a few minutes)...")
    t0 = time.monotonic()
    ...
    log.info("Core + worker images built in %.0fs", elapsed)

    log.info("Building kernel image...")
    t0 = time.monotonic()
    ...
    log.info("Kernel image built in %.0fs", elapsed)

    # -- Step 3: generate one-time secrets --
    log.info("Generating one-time secrets...")
    env = {
    ...
    try:
        ...
        log.info("Starting flowfile-core and flowfile-worker...")
        up = _compose("--env-file", env_file.name, "up", "-d", "flowfile-core", "flowfile-worker")
        if up.returncode != 0:
            pytest.fail(f"docker compose up failed:\n{up.stderr}")

        try:
            log.info("Waiting for core to become healthy (%s)...", CORE_URL)
            if not _wait_for_service(CORE_URL, "core", timeout=120):
            ...
            log.info("All services healthy — running tests")
            yield env
        finally:
            # -- Step 11: teardown (always runs) --
            log.info("Tearing down compose services...")
...
def auth_client(compose_services):
    ...
    log.info("Authenticating as %s...", env["FLOWFILE_ADMIN_USER"])
...
def kernel_ready(auth_client):
    ...
    log.info("Creating kernel '%s'...", kernel_id)
    ...
    # Step 7: start kernel
    log.info("Starting kernel '%s'...", kernel_id)
    resp = auth_client.post(f"/kernels/{kernel_id}/start")
    resp.raise_for_status()

    # Wait for kernel to become idle
    log.info("Waiting for kernel to become idle (up to 120s)...")
    ...
    while time.monotonic() < deadline:
        ...
        if state == "idle":
            log.info("Kernel idle after %d polls", attempt)
            break
        if attempt % 5 == 0:
            log.info("Kernel state: %s (poll %d)", state, attempt)
        time.sleep(2)
    else:
    ...
    log.info("Stopping kernel '%s'...", kernel_id)
    try:
        auth_client.post(f"/kernels/{kernel_id}/stop")
    except Exception:
        pass
    log.info("Deleting kernel '%s'...", kernel_id)
    try:
