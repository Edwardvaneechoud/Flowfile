...
- `src/renderer/app/pages/nodeDesigner/` — the Node Designer: `designerState.ts` is a 1:1 TS mirror of core's `node_designer/state.py` (the wire contract — the frontend edits `DesignerState` and **never generates Python**; save/preview/dry-run go through `api/nodeDesigner.ts`, backend renders the `.py`). `designer/` holds the page components (MetadataPanel with category combobox, WorkspaceTabs Form|Code|Test, FormCanvas WYSIWYG hosting the shared `customNode/CustomNodeForm.vue` in editMode, ControlInspector, ExecutionEnvironmentPicker, FormFieldsOverview, testPanel/). State in `stores/node-designer-store.ts` (per-node localStorage drafts, dirty guards, body-only `processBody` under a read-only signature header). Custom-node icons are JWT-gated — always resolve via `app/composables/useCustomNodeIcon.ts` (authed blob), never raw `<img src>` endpoint URLs. The Catalog's Custom Nodes tab (`views/CatalogView/CustomNodesPanel.vue`) deep-links here via `?openFile=`.
